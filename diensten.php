...
// Get the selected category from the GET parameter, defaulting to null if not set
$GekozenCategorie = isset($_GET['categorie']) ? $_GET['categorie'] : null;

// Prepare and execute the SQL query for products
if ($GekozenCategorie) {
    $Sql = "SELECT naam, foto FROM products WHERE categorie = ?";
    $Stmt = $Conn->prepare($Sql);
    $Stmt->bind_param("s", $GekozenCategorie);
} else {
    $Sql = "SELECT naam, foto FROM products";
    $Stmt = $Conn->prepare($Sql);
}
            ...
            <select name="categorie" id="categorie" onchange="this.form.submit()">
                ...
                <?php foreach ($Categorieen as $Categorie): ?>
                    <option value="<?php echo htmlspecialchars($Categorie['categorie']); ?>"
                        <?php echo ($Categorie['categorie'] == $GekozenCategorie) ? 'selected' : ''; ?>>
        ...
        <div class="product-1">
            <div class="title">
                <div class="section-title">
                    ...
                    <div class="content2">
                        <h1 class="heading">
                            <?php if ($GekozenCategorie): ?>
                                Producten in categorie: <?php echo htmlspecialchars($GekozenCategorie); ?>
                            <?php else: ?>
                                Alle producten
                            <?php endif; ?>
                        </h1>
                        <div class="text">Bekijk onze producten.</div>
                    </div>
                </div>
